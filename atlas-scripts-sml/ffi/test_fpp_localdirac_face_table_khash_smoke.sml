...
(* Smoke test for `FPP_localDirac.local_face_table_khash_limit`.

   We pick the first `x` with at least one fixture lambda, build a tiny face
   table with `maxPerDim=2`, and sanity-check the row shapes and hash indices. *)
...
val fd = FPP_localDirac.local_face_table_khash_limit (g, x, lambda, 2, polHash);
