(* Functional smoke test: build W(E6) character table and check orthogonality. *)
...
fun main () =
  let
    val ct = CharacterTable_E6.character_table_E6_magma ()
    val () = expect ("|W(E6)|", CharacterTables.order_W ct, 51840)
    val () = CharacterTables.assert_orthogonality ct
  in
    print "test_character_table_E6_functional_smoke: ok\n"
  end
