...
(* Smoke test for `FPP_localDirac.local_testK_hash_simple_limit`.

   This is intentionally tiny (`maxFacesPerDim=1`) since it triggers exact
   unitarity checks. We just sanity-check the returned table shape. *)
...
val fd = FPP_localDirac.local_testK_hash_simple_limit (g, x, lambda, 1, polHash);
