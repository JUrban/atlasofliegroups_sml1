...
(* Smoke test for `.at`-compatibility entry points `local_test_GEO_hash_dumb*`. *)
...
val ps = FPP_localDirac.local_test_GEO_hash_dumb_limit (g, x, lambda, 5);
val () = if length ps > 0 then () else raise Fail "expected nonempty list";
...
val added = FPP_localDirac.local_test_GEO_hash_dumb_into_hash_limit (g, x, lambda, 5, uhash);
